...
class PConvUnet(object):
    ...
    #     """
    #     Load pre-trained VGG16 from Keras applications.
    #     Extract features to be used in the loss function.
    #     """

    #     # Input image with the correct shape (512, 512, 3)
    #     img = Input(shape=(self.img_rows, self.img_cols, 3))  # No extra batch size here

    #     # Mean center and rescale by variance (consistent with PyTorch preprocessing)
    #     processed = Lambda(lambda x: (x - self.mean) / self.std)(img)

    #     # If only inference is required, return a placeholder model
    #     if self.inference_only:
    #         model = Model(inputs=img, outputs=[img for _ in range(len(self.vgg_layers))])
    #         model.trainable = False
    #         model.compile(loss='mse', optimizer='adam')
    ...
    #     vgg = VGG16(weights=weights, include_top=False, input_shape=(self.img_rows, self.img_cols, 3))

    #     # Extract outputs from the specified layers
    #     # vgg_outputs = [vgg.layers[i].output for i in self.vgg_layers]
    #     vgg = Model(vgg.inputs, [vgg.layers[i].output for i in self.vgg_layers])
